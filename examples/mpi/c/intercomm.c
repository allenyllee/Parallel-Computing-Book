...
#define CHK(x) if (x) {							\
    ...
    fprintf(stderr,"p=%d, line=%d, err=%d, %s\n",procno,__LINE__,x,errtxt);	\
    return x;}
#define ASSERT(p) if (!(p)) {fprintf(stderr,"Assertion failed for proc %d at line %d\n",procno,__LINE__); return -1;}
#define ASSERTm(p,m) if (!(p)) {fprintf(stderr,"Message<<%s>> for proc %d at line %d\n",m,procno,__LINE__); return -1;}
