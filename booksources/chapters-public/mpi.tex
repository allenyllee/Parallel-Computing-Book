...
came from, you would use the \indexmpishow{MPI_SOURCE} field of the receive status.

The following code fragment models a scenario where multiple workers tasks
compute something (in this case they simply wait a random time) and then report
to the root process. The root takes the first incoming message, and tells the workers
which of them was accepted; all others then cancel their message\indexmpi{MPI_Cancel}.
