...
Distributed: no master, but there is an \n{mpirun} program.
\ac{SPMD} mode: all processors execute the same program.
...
passive RMA. In \indextermsub{active}{RMA}, both parties are involved; the main advantage
...
In \indextermsub{passive}{RMA}, the target process is not involved. 
...
Two fences delineate a so-called \indexterm{epoch}. 
...
\Level 2 {Active and passive target synchronization}
...
simplest type has a \indextermsub{root of the}{collective} where
information originates or is collected. Then there is an `all' variant
of operations such as the reduction, where the result is not left just
on the root but everywhere. Finally, there are `v' variants where the
...
\begin{exercise}
  Extend your Mandelbrot code to use collectives, as outlined
  in section~\ref{proj:mandel-collective}.
\end{exercise}
...
The main mechanisms for making new communicators are the following:
\begin{itemize}
\item \indexmpishow{MPI_Comm_dup} makes an exact duplicate of a communicator.
  This is useful for software libraries.
\item \indexmpishow{MPI_Comm_split} takes a single communicator and splits it 
  into multiple, disjoint, new ones. This uses a `colour':
\begin{verbatim}
...
\item The more general mechanism is based on groups: you can extract the 
