...
This status is a property of the actually received messsage, so \lstinline$MPI_Irecv$
does not have a status parameter, but \lstinline$MPI_Wait$ does.
...
that is delivered by \lstinline$MPI_Recv$ or the \lstinline$MPI_Wait...$ call after an \lstinline$MPI_Irecv$.
\begin{lstlisting}
...
\end{lstlisting}
This may be necessary since the \n{count} argument to \lstinline$MPI_Recv$ is 
...
An \lstinline$MPI_Isend$ or \lstinline$MPI_Irecv$ call has an \indexmpishow{MPI_Request} parameter.
This
...
\item You describe the communication with
  ...
  \lstinline$MPI_Isend$, or \indexmpishow{MPI_Recv_init}, which has the same
  calling sequence as \lstinline$MPI_Irecv$.
...
%
These are equivalent to starting an \lstinline$MPI_Isend$ or \lstinline$MPI_Isend$; correspondingly, 
it is necessary to issue an \lstinline$MPI_Wait...$ call (section~\ref{sec:nonblocking})
...
\begin{itemize}
\item \lstinline$MPI_SUCCESS$ the routine completed successfully.
...
unlike \lstinline$MPI_STATUS$. Instead it is an (opaque) pointer.
This meeans that when you call, for instance, \lstinline$MPI_Irecv$,
MPI will allocate an actual request object, and return its
address in the \lstinline$MPI_Request$ variable.
