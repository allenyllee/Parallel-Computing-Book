...
\begin{exercise}
  ...
  and compute the sum of these numbers using the \lstinline$MPI_Allreduce$
  routine.

  (The operator is \lstinline$MPI_SUM$ for C/Fortran, or \lstinline$MPI.SUM$ for
  Python.)
...
for now you can use \lstinline$MPI_COMM_WORLD$.
...
\item A buffer. In~C this is the address in memory of the data. This means
  that you broadcast a single scalar as \lstinline$MPI_Bcast$\n{( &value, ... )},
  but an array as \lstinline$MPI_Bcast$\n{( array, ... )}.
\item The number of items and their datatype. The allowable datatypes
  are such things as \lstinline$MPI_INT$ and \lstinline$MPI_FLOAT$ for~C, and
  \lstinline$MPI_INTEGER$ and \lstinline$MPI_REAL$ for Fortran, or more complicated types.
