...
$3\times3$ cells and returns the updated status of the center cell:
\begin{verbatim}
...
The above code created a board for all time steps, which
...
and we will now explore what \ac{SIMD} parallelism looks like
...
called \indexterm{loop unrolling}, specifically, unrolling by~2.
...
\begin{enumerate}
\item first bring them on the same power of ten: $0.58\times 10^2+3.2\times 10^2$,
...
until NVIDIA came out with the \indexacf{CUDA} language.
\ac{CUDA} is a way of explicitly doing data parallel programming:
you write piece of code called a \indexterm{kernel}, which 
...
figure~\ref{fig:send-blocking}. Practice is different.
...
\begin{exercise}
  Life expresses the value in \n{i,j} at time $t+1$ as a simple function of the
  $3\times3$ patch \n{i-1:i+1,j-1:j+1} at time~$t$.  Convince yourself that the
  value in \n{i,j} at~$t+\nobreak2$ can be computed as a function of a
  $5\times5$ patch at~$t$.

  Can you formulate rules for this update over two timesteps? Are these rules as
  elegant as the old ones, just expressed in a count of live and dead
  cells? If you would code the new rules as a case statement, how many 
  clauses would there be? Let's not persue this further\ldots
\end{exercise}
...
\begin{exercise}
  Just how independent can processors be? If processor $i,j$ is working on time~$t$,
  can processor $i+1,j$ be working on $t+2$? Can you give a formal description of
  how far out of step processor $i,j$ and $i',j'$ can be?
\end{exercise}
...
However, in many cases a more refined analysis gives the same conclusion:
...
\item As you buy more processors, you can store a larger Life board:
  we're assuming that the amount $M$ of memory is kept constant. (This
...
Let's analyze the cost of this scheme.
