...
(section~\ref{sec:mpi:error}) is \lstinline$MPI_ERR_COMM$, which can be returned
...
old \lstinline$MPI_COMM_WORLD$, so it is better to set up your code as a
...
\item \indexmpidef{MPI_UNIVERSE_SIZE}: the total number of processes
  ...
  \lstinline$MPI_COMM_WORLD$ if the host list is larger than the number of
...
For instance, people have inserted dummy \lstinline$MPI_Probe$ calls.
...
case, replacing every \lstinline$MPI_Send$ with a \indexmpishow{MPI_Ssend} will force the
...
complete is needed. Thus, we issue an \lstinline$MPI_Recv$
...
MPI constants such as \lstinline$MPI_COMM_WORLD$ or \lstinline$MPI_INT$ are not
...
with \lstinline$MPI_Query_thread$:
%
...
\Level 0 {Shell scripting cuteness}
...
This sort of script can also be used to implement \indexac{MPMD} runs:
