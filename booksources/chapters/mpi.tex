...
\Level 1 {Concepts: blocking vs non-blocking, synchronous vs asynchronous}
...
figure~\ref{fig:send-blocking}, left picture.
\begin{figure}[ht]
\hbox{%
\includegraphics[scale=.08]{graphics/send-blocking}
\kern1cm
\includegraphics[scale=.08]{graphics/send-nonblocking}
}
\caption{Left: illustration of a blocking communication: the sending
  processor is idle until the receiving processor issues and finishes
  the receive call.  Right: illustration of a non-blocking
  communication: the sending processor immediately continues execution
  after issueing the send call}
...
or send call is successfully concluded. However, this can be inefficient;
it is better if the data is set aside for the system to transfer it, 
while the user program continues. This is known as \emph{non-blocking}
communication, illustrated in the right figure.
...
with \indexmpishow{MPI_Op_create}; see section~\ref{mpi:op-create}
