...
a combination of others. You have already seen \lstinline$MPI_Allreduce$ which
...
\item The big trick is that each process now knows how many of these
  ...
  \lstinline$MPI_Irecv$ calls, with a source of \indexmpishow{MPI_ANY_SOURCE}.
\end{itemize}
...
The solution is to use \lstinline$MPI_Reduce_scatter$ to find out how many processes
...
Use of \lstinline$MPI_Reduce_scatter$ to implement the two-dimensional
matrix-vector product.
Set up separate row and column communicators with
\indexmpishow{MPI_Comm_split}, use \lstinline$MPI_Reduce_scatter$ to combine
local products.
%
