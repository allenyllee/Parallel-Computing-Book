...
\begin{exercise}
  \label{ex:rowcolcomm}
  Organize your processors in a grid, and make subcommunicators for
  the rows and columns. Do a broadcast from the first row and column
  through the columns and rows respectively.

  If you let the broadcast value be the column/row number, then
  processor $(i,j)$ winds up with the numbers $i$~and~$j$. Test this.
\end{exercise}
