...
the same for all). In the regular \lstinline$MPI_Gather$ call the root processor
...
All the processors specify a send buffer just as with \lstinline$MPI_Gather$.
...
to find these individual amounts with an \lstinline$MPI_Gather$ call, and
