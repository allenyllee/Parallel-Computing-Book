...
use petscXXX
\end{verbatim}
(here \n{XXX} stands for one of the PETSc types, but ommision gives
inclusion of the whole library.)

Variables can be declared with their type (\n{Vec}, \n{Mat}, \n{KSP}
et cetera), but internally they are Fortran \lstinline{Type} objects
so they can be declared as such.
...
\petscRoutineRef{PetscInitialize}
...
\indexpetscdef{PetscPrintf}.
%
\petscRoutineRef{PetscPrintf}

If all processes need to print, you can use
\indexpetscdef{PetscSynchronizedPrintf} that forces the
...
use \indexpetscdef{PetscSynchronizedFlush}
%
