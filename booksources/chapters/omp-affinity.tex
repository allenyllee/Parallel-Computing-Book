...
\begin{itemize}
\item the environment variable \indextermttdef{OMP_PROC_BIND}
  describes how threads are bound to \indextermbus{OpenMP}{places}; while
\item the variable \indextermttdef{OMP_PLACES} describes these places
  in terms of the available hardware.
\item When you're experimenting with these variables it is a good idea
  to set \indextermttdef{OMP_DISPLAY_ENV} to true, so that OpenMP will
...
The variable \indextermttdef{OMP_PLACES} defines a series of places to
...
There are three predefined values for the \indextermtt{OMP_PLACES}
